...
export default function SupportButton() {
  ...
  useEffect(() => {
    ...
    const fetchInfo = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${CLIENT_API_URL}/api/public/support-info`);
        if (!res.ok) return;
        ...
        // silencioso — usa fallback ou cache
      } finally {
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 relative">
            ...
            {loading && (
              <p className="text-xs text-gray-400 mb-3 flex items-center gap-1">
                <Loader2 className="w-3 h-3 animate-spin" /> Atualizando informações…
              </p>
            )}

            <div className="space-y-3 mt-4">
              <a
                href={`mailto:${info.email || FALLBACK.email}`}
              ...
              <a
                href={`https://wa.me/${(info.whatsapp || FALLBACK.whatsapp).replace(/\D/g, '')}`}
                target="_blank"
