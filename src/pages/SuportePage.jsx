...
function NovoTicket({ onCreated, onCancel }) {
  ...
  const submit = async (e) => {
    ...
    setError(null);
    try {
      const res = await apiFetch(`${CLIENT_API_URL}/api/support/tickets`, {
        method: 'POST',
        headers: headers(),
...
function TicketDetalhe({ ticketId, onBack }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await apiFetch(`${CLIENT_API_URL}/api/support/tickets/${ticketId}`, { headers: headers() });
  ...
  const send = async (e) => {
    ...
    setSending(true);
    try {
      const res = await apiFetch(`${CLIENT_API_URL}/api/support/tickets/${ticketId}/messages`, {
        method: 'POST',
        headers: headers(),
  ...
  const closeTicket = async () => {
    ...
    await apiFetch(`${CLIENT_API_URL}/api/support/tickets/${ticketId}/status`, {
      method: 'PATCH',
      headers: headers(),
  ...
  if (loading) return <div className="flex justify-center py-10"><Loader2 className="animate-spin h-8 w-8 text-orange-500" /></div>;
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div>
            ...
            <p className="text-xs text-gray-500">
              {ticket.category} • {ticket.priority} • Aberto {new Date(ticket.created_at).toLocaleDateString('pt-BR')}
            </p>
          </div>
          ...
          {messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-2xl px-4 py-2 ${isMe ? 'bg-orange-100 text-orange-900' : 'bg-gray-100 text-gray-800'}`}>
        ...
        {ticket.status !== 'resolvido' ? (
          <>
            <form onSubmit={send} className="flex gap-2 mt-4">
              ...
              />
              <button type="submit" disabled={sending || !body.trim()} className="bg-orange-500 text-white rounded-full p-2 disabled:opacity-50">
          ...
          <div className="mt-4 p-3 bg-emerald-50 text-emerald-700 text-sm rounded-lg text-center">
...
export default function SuportePage() {
  ...
  const fetchTickets = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiFetch(`${CLIENT_API_URL}/api/support/tickets`, { headers: headers() });
      const json = await res.json();
  ...
  useEffect(() => {
    fetchTickets();
    fetch(`${CLIENT_API_URL}/api/public/support-info`)
  ...
  if (view === 'detail' && selected) {
    return (
      <div className="container mx-auto px-4 py-6 max-w-3xl">
        <TicketDetalhe ticketId={selected} onBack={() => { setSelected(null); setView('list'); fetchTickets(); }} />
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-3xl space-y-4">
      <div className="flex justify-between items-center gap-3">
        ...
        <button onClick={() => setView('new')} className="flex items-center gap-1 bg-orange-500 text-white px-3 py-2 rounded-lg text-sm font-bold">
          <Plus className="w-4 h-4" /> Novo chamado
        </button>
      </div>

      <div className="grid sm:grid-cols-2 gap-3">
        <a href={`mailto:${supportInfo.email}`} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-3 hover:border-orange-200 hover:shadow-sm transition-all">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="p-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-800">Meus chamados</h2>
        </div>
        {loading ? (
          <div className="flex justify-center py-10"><Loader2 className="animate-spin h-8 w-8 text-orange-500" /></div>
        ) : tickets.length === 0 ? (
          <div className="text-center py-10 px-4">
            <MessageCircle className="w-10 h-10 text-gray-300 mx-auto mb-2" />
            <p className="text-sm text-gray-500">Você ainda não abriu nenhum chamado.</p>
            <button onClick={() => setView('new')} className="mt-3 text-sm font-medium text-orange-600 hover:underline">
              Abrir o primeiro
            </button>
          </div>
        ) : (
          <div className="divide-y divide-gray-50">
            {tickets.map((t) => {
              ...
              return (
                <button
                  key={t.id}
                  onClick={() => { setSelected(t.id); setView('detail'); }}
                  className="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center justify-between gap-3"
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{t.subject}</p>
                    <p className="text-xs text-gray-500">
                      {t.category} • {new Date(t.updated_at).toLocaleDateString('pt-BR')} • {t.messages_count} {t.messages_count === 1 ? 'mensagem' : 'mensagens'}
                    </p>
                  </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <div className="flex flex-col gap-2 text-sm">
          <a href="https://inksadelivery.com.br/termos" target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:underline">Termos de Uso</a>
          <a href="https://inksadelivery.com.br/privacidade" target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:underline">Política de Privacidade</a>
          <a href="https://inksadelivery.com.br/excluir-conta" target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:underline">Excluir minha conta</a>
          <a href="https://inksadelivery.com.br/excluir-dados" target="_blank" rel="noopener noreferrer" className="text-orange-600 hover:underline">Exclusão de dados</a>
        </div>
      </div>
    </div>
  );
