...
const MyOrdersPage = () => {
    ...
    const { addToast } = useToast(); // Desestruturar addToast

    const fetchOrders = useCallback(async () => {
        try {
            const authToken = AuthService.getToken(); 
            if (!authToken) {
                // AuthService.logout() já redireciona para /login
                AuthService.logout(); 
    ...
    }, [navigate, addToast]); // Adicionar addToast nas dependências

    useEffect(() => {
        fetchOrders();
        // Manter o polling para atualização de status, se desejar
    ...
    // ✅ NOVO: Função para deletar um pedido
        ...
        return orders.map(order => (
            <div key={order.id} className="bg-white p-6 rounded-lg shadow-md border border-gray-200 mb-6">
                <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2"> {/* Container para status e botão */}
                        <span className={`px-3 py-1 text-sm font-bold rounded-full ${getStatusClasses(order.status)}`}>
                            {order.status}
                        </span>
                        {/* ✅ NOVO: Botão de exclusão */}
                        {/* Pedidos Concluídos ou Cancelados podem ser excluídos */}
                ...
                <div className="mt-4 border-t border-gray-100 pt-4">
                    <p className="text-gray-700"><strong>Valor Total:</strong> R$ {parseFloat(order.total_amount).toFixed(2)}</p>
                    <p className="text-gray-700"><strong>Data:</strong> {new Date(order.created_at).toLocaleString('pt-BR')}</p>
                </div>
            </div>
        ));
    };
